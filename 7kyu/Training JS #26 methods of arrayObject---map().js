...
const isolateIt = (arr) => {
    return arr.map((string) => {
        if (string.length % 2 === 0) return string.slice(0, string.length / 2) + '|' + string.slice(string.length / 2)
        else if (string.length % 2 !== 0) return string.slice(0, (string.length - 1) / 2) + '|' + string.slice((string.length + 1) / 2)
    })
}
